# Media Crawler — 系统设计分析

> 本文档是对 `media-crawler` 项目的设计思路分析，聚焦于架构决策、反爬策略设计与工程化实践。
> 代码实现细节已脱敏，不包含目标网站信息或可被滥用的具体技术参数。
> 仅供技术交流与求职展示使用
...
>数据处理管道的示例（清洗-统计-可视化，详见 [data/pipeline.py](https://github.com/Drift-bottle/media-crawler-analysis/blob/main/data/pipeline.py)）：
...
| 类别 | 技术组件 | 选型理由（简要）                                  |
| :--- | :--- |:------------------------------------------|
| **异步网络层** | `httpx` | 支持异步、连接池复用、Cookie 持久化、事件钩子（用于耗时监控）        |
| **浏览器自动化** | `playwright` | 用于通过 CDP 连接 Edge 获取真实登录态 Cookie，避免手动模拟登录  |
| **重试与容错** | `tenacity` | 提供灵活的重试装饰器，支持异常类型过滤、指数退避、自定义回调，便于实现分层重试策略 |
| **数据统计** | `collections.Counter` | 标准库，无额外依赖，满足词频统计需求                        |
| **数据可视化** | `wordcloud` + `matplotlib` | 组合生成词云图与条形图，输出高清组合图像，适合用于展示               |
| **测试框架** | `pytest` + `pytest-asyncio` | 支持异步测试，与项目全异步架构匹配                         |
...
### 2. 人机验证处理有限
当前代码**未集成自动打码方案或人机验证识别模型**，若触发风控并下发特定字段，会抛出异常并进入重试流程。若重试耗尽仍失败，爬虫将**直接结束整个爬取流程**，不再请求后续分段，随后进入数据保存与可视化阶段。
...
欢迎围绕文档已描述的内容展开技术讨论，包括设计思路的探讨、事实指正、替代方案补充等。

**以下内容不属于讨论范围：**

- 求完整代码、求可用配置、求现成爬虫工具
- 与文档内容无关的独立技术问题（例如你自己项目里的报错、环境配置问题）
- 询问如何绕过特定网站的具体反爬机制
...
>说明：本文档仅描述项目的设计架构，不反映项目当前运行状态与维护情况。